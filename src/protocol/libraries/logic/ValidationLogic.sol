...
library ValidationLogic {
    ...
    function validateDeposit(DataTypes.ReserveData storage reserve, uint256 amount) external view {
        ...
        uint256 maxCapacity =
            IReserveInterestRateStrategy(reserve.interestRateStrategyAddress).reserveCapacity();
        ...
        if (maxCapacity > 0) {
            ...
            if (isCollateral && reserve.yieldAddress != address(0)) {
                ...
                if (decimal < 18) {
                    depositAmount =
                        amount.mul(10 ** (18 - decimal)).rayMul(reserve.getIndexFromPricePerShare());
                } else {
                    depositAmount = amount.rayMul(reserve.getIndexFromPricePerShare());
                }
            }
     ...
     * @param maxStableLoanPercent The max amount of the liquidity that can be borrowed at stable
     * rate, in percentage
        ...
        view
    {
        ...
        //add the current already borrowed amount to the amount requested to calculate the total
        // collateral needed.
        vars.amountOfCollateralNeededETH =
            vars.userBorrowBalanceETH.add(amountInETH).percentDiv(vars.currentLtv); //LTV is calculated
            // in percentage
        ...
        view
    {
        ...
        uint256 usageRatio =
            totalDebt == 0 ? 0 : totalDebt.rayDiv(availableLiquidity.add(totalDebt));

        //if the liquidity rate is below REBALANCE_UP_THRESHOLD of the max variable APR at 95%
        // usage,
