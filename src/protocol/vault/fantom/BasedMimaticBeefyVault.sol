...
contract BasedMimaticBeefyVault is GeneralVault {
        ...
        returns (uint256)
    {
        ...
        require(
            msg.sender == _addressesProvider.getLendingPool(), Errors.LP_LIQUIDATION_CALL_FAILED
        );
        ...
        returns (uint256)
    {
        ...
        uint256 minTotalPrice = _amount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("mooTombBASED-MIMATIC"))
        ).div(2).percentMul(9900);

        uint256 minMiMaticAmountFromPrice =
            minTotalPrice.div(oracle.getAssetPrice(_addressesProvider.getAddress("MIMATIC")));

        uint256 minBasedAmountFromPrice =
            minTotalPrice.div(oracle.getAssetPrice(_addressesProvider.getAddress("BASED")));
        ...
        uint256 minAmountFromPrice = minMiMaticAmountFromPrice.percentMul(9800);
        ...
        uint256[] memory receivedAmounts = IUniswapV2Router02(tombSwapRouter)
            .swapExactTokensForTokens(
            amountBASED, minAmountFromPrice, path, address(this), block.timestamp
        );
        ...
        internal
    {
        ...
        (uint256 minAmount, address[] memory path) =
            _getPathAndMinAmount(_tokenIn, _tokenOut, _tokenAmount);

        IERC20(_tokenIn).approve(uniswapRouter, _tokenAmount);

        uint256[] memory receivedAmounts = IUniswapV2Router02(uniswapRouter)
            .swapExactTokensForTokens(_tokenAmount, minAmount, path, address(this), block.timestamp);
        ...
        IERC20(_tokenOut).safeApprove(
            address(_addressesProvider.getLendingPool()), receivedAmounts[path.length - 1]
        );
    ...
    function _getPathAndMinAmount(
        ...
        internal
        returns (uint256 minAmount, address[] memory path)
    {
        ...
        uint256 minTotalPrice =
            _tokenAmount.mul(oracle.getAssetPrice(_tokenIn)).div(10 ** inputAssetDecimal);

        uint256 minAmount = minTotalPrice.mul(10 ** outputAssetDecimal).div(
    ...
    function pricePerShare() external view override returns (uint256) {
        return IBeefyVault(_addressesProvider.getAddress("mooTombBASED-MIMATIC"))
    ...
    function _processTreasury(uint256 _yieldAmount) internal returns (uint256) {
        ...
        IERC20(_addressesProvider.getAddress("mooTombBASED-MIMATIC")).safeTransfer(
            _treasuryAddress, treasuryAmount
        );
