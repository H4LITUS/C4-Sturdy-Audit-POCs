...
library CurveswapAdapter {
        ...
        returns (uint256)
    {
        ...
        // Approves the transfer for the swap. Approves for 0 first to comply with tokens that
        // implement the anti frontrunning approval fix.
        ...
        uint256 receivedAmount = ICurveExchange(curveExchange).exchange(
            poolAddress, assetToSwapFrom, assetToSwapTo, amountToSwap, minAmountOut, address(this)
        );
        ...
        returns (uint256)
    {
        ...
        uint256 minAmountOut = amountToSwap.mul(fromAssetPrice.mul(10 ** toAssetDecimals)).div(
