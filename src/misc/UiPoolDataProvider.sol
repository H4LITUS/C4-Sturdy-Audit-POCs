...
contract UiPoolDataProvider is IUiPoolDataProvider {
    ...
    constructor(
        ISturdyIncentivesController _incentivesController,
    ...
    function getSimpleReservesData(ILendingPoolAddressesProvider provider)
        ...
        view
        override
        returns (AggregatedReserveData[] memory, uint256, uint256)
    {
        ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            if (isCollateral && baseData.yieldAddress != address(0)) {
                ...
                if (decimal < 18) {
                    reserveData.availableLiquidity = reserveData.availableLiquidity.mul(
                        10 ** (18 - decimal)
                    ).rayMul(pricePerShare);
                } else {
                    reserveData.availableLiquidity =
                        reserveData.availableLiquidity.rayMul(pricePerShare);
                }
            }
            ...
            // we're getting this info from the aToken, because some of assets can be not compliant
            // with ETC20Detailed
        ...
        returns (UserReserveData[] memory, uint256)
    {
      ...
      user != address(0) ? reserves.length : 0
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            // incentives
            if (address(0) != address(incentivesController)) {
                userReservesData[i].aTokenincentivesUserIndex =
                    incentivesController.getUserAssetData(user, baseData.aTokenAddress);
                userReservesData[i].vTokenincentivesUserIndex =
                    incentivesController.getUserAssetData(user, baseData.variableDebtTokenAddress);
                userReservesData[i].sTokenincentivesUserIndex =
                    incentivesController.getUserAssetData(user, baseData.stableDebtTokenAddress);
            }
            // user reserve data
      ...
      user != address(0) ? reserves.length : 0
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            if (user != address(0)) {
                ...
                if (address(0) != address(incentivesController)) {
                    userReservesData[i].aTokenincentivesUserIndex =
                        incentivesController.getUserAssetData(user, reserveData.aTokenAddress);
                    userReservesData[i].vTokenincentivesUserIndex = incentivesController
                        .getUserAssetData(user, reserveData.variableDebtTokenAddress);
                    userReservesData[i].sTokenincentivesUserIndex = incentivesController
                        .getUserAssetData(user, reserveData.stableDebtTokenAddress);
                }
                // user reserve data
                userReservesData[i].underlyingAsset = reserveData.underlyingAsset;
                userReservesData[i].scaledATokenBalance =
                    IAToken(reserveData.aTokenAddress).scaledBalanceOf(user);
                userReservesData[i].usageAsCollateralEnabledOnUser =
        ...
        if (address(0) != address(incentivesController)) {
            if (user != address(0)) {
                incentivesControllerData.userUnclaimedRewards =
