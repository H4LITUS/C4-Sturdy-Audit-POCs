...
contract TombFtmBeefyVault is GeneralVault {
    ...
    function processYield() external override onlyYieldProcessor {
        ...
        (uint256 yieldTOMB, uint256 yieldFTM) =
            _withdrawLiquidityPool(TOMB_FTM_LP, yieldTOMB_FTM_LP);
        ...
        returns (uint256 amountToken, uint256 amountFTM)
    {
        ...
        uint256 minTotalPrice = _amount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("mooTombTOMB-FTM"))
        ).div(2).percentMul(9900);

        uint256 minFTMAmountFromPrice =
            minTotalPrice.div(oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM")));

        uint256 minTokenAmountFromPrice =
    ...
    function _convertAndDepositTokenYield(address _tokenOut, uint256 _tombAmount) internal {
        ...
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = _tombAmount.mul(oracle.getAssetPrice(TOMB)).div(10 ** 18).mul(
            10 ** assetDecimal
        ).div(oracle.getAssetPrice(_tokenOut)).percentMul(9800);
        ...
        uint256[] memory receivedAmounts = IUniswapV2Router02(uniswapRouter)
            ...
            _tombAmount, minAmountFromPrice, path, address(this), block.timestamp
        );
        ...
        IERC20(_tokenOut).safeApprove(
            address(_addressesProvider.getLendingPool()), receivedAmounts[2]
        );
    ...
    function _convertAndDepositYield(address _tokenOut, uint256 _ftmAmount) internal {
        ...
        uint256 assetDecimal = IERC20Detailed(_tokenOut).decimals();
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = _ftmAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(_tokenOut)).percentMul(
            9900
        );
        ...
        // Make lendingPool to transfer required amount
        IERC20(_tokenOut).safeApprove(
            address(_addressesProvider.getLendingPool()), receivedAmounts[1]
        );
        // Deposit yield to pool
