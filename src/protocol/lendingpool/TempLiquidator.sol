...
contract TempLiquidator is IFlashLoanReceiver, Ownable {
    ...
    {
        // parse params
        (address collateralAddress, address borrowerAddress) =
        ...
        ILendingPool(_addressesProvider.getLendingPool()).liquidationCall(
            collateralAddress, assets[0], borrowerAddress, amounts[0], false
        );
    ...
    function liquidation(address debtAsset, uint256 debtToCover, bytes calldata params) external {
        IAaveFlashLoan AAVE_LENDING_POOL =
            IAaveFlashLoan(_addressesProvider.getAddress("AAVE_LENDING_POOL"));
        ...
        internal
    {
        ...
        uint256 minAmountFromPrice = collateralAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(asset)).percentMul(9900);
        ...
        internal
    {
        ...
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = collateralAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVBOO"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(asset)).percentMul(9800);
        ...
        uint256[] memory receivedAmounts = IUniswapV2Router02(uniswapRouter)
            .swapExactTokensForTokens(
            collateralAmount, minAmountFromPrice, path, address(this), block.timestamp
        );
        ...
        internal
    {
        ...
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = collateralAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVSPELL"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(asset)).percentMul(9800);
        ...
        internal
    {
        ...
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = collateralAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVCRV"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(asset)).percentMul(9800);
    ...
    {
        // burn fBEETS token
        ICollateralAdapter collateralAdapter =
            ICollateralAdapter(_addressesProvider.getAddress("COLLATERAL_ADAPTER"));
        ...
        bytes32 beethoven_BEETS_FTM_PoolId =
            IYearnFBEETSVault(fBEETSVault).beethoven_BEETS_FTM_PoolId();
        require(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            amountsOut[i] = balances[i].mul(_amount).div(_totalAmount).percentMul(9900);
        }
        ...
        IBalancerVault(IYearnFBEETSVault(fBEETSVault).getBeethovenVault()).exitPool(
            ...
            IBalancerVault.ExitPoolRequest({
                ...
                userData: abi.encode(
                    IBalancerWeightedPool.ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT, _amount
                    ),
        ...
        returns (uint256)
    {
        ...
        // Calculate minAmount from price with 2% slippage
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = beetsAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("BEETS"))
        ).div(10 ** 18);
        minAmountFromPrice = minAmountFromPrice.mul(10 ** assetDecimal).div(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))
        ).percentMul(9800);
        ...
        returns (uint256)
    {
        ...
        // ToDo: Need to consider batchSwap, but, it's impossible now to implement Smart Order
        // Router on-chain
        ...
        internal
    {
        ...
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = collateralAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVLINK"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(oracle.getAssetPrice(asset)).percentMul(9800);
