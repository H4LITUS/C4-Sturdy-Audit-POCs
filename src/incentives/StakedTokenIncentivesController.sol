 ...
 * @notice Distributor contract for rewards to the Sturdy protocol, using a staked token as rewards
 * asset.
    ...
    DistributionManager
{
    ...
    // useful for contracts that hold tokens to be rewarded but don't have any native logic to claim
        ...
        onlyEmissionManager
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            require(
                assetsConfig[i].emissionPerSecond == emissionsPerSecond[i], "INVALID_CONFIGURATION"
            );
        ...
        override
    {
        uint256 accruedRewards =
            _updateUserAssetInternal(user, msg.sender, userBalance, totalSupply);
        if (accruedRewards != 0) {
    ...
    {
        return (
            assets[asset].index, assets[asset].emissionPerSecond, assets[asset].lastUpdateTimestamp
        );
    }
        ...
        returns (uint256)
    {
      ...
      assets.length
        ...
        _usersUnclaimedRewards[user] = unclaimedRewards - amountToClaim; // Safe due to the previous
            // line
