...
import { VersionedInitializable } from
    "../libraries/sturdy-upgradeability/VersionedInitializable.sol";
 ...
 * @dev Implements actions involving management of collateral in the protocol, the main one being
 * the liquidations
 * IMPORTANT This contract will run always via DELEGATECALL, through the LendingPool, so the chain
 * of inheritance
    ...
    LendingPoolStorage
{
        ...
        returns (uint256, string memory)
    {
        ...
        if (!receiveAToken) {
            uint256 currentAvailableCollateral =
                IERC20(internalAsset).balanceOf(address(vars.collateralAtoken));
        ...
        if (vars.userVariableDebt >= vars.actualDebtToLiquidate) {
            ...
            IStableDebtToken(debtReserve.stableDebtTokenAddress).burn(
                user, vars.actualDebtToLiquidate.sub(vars.userVariableDebt)
            );
        }
        ...
        if (receiveAToken) {
            ...
            collateralReserve.updateInterestRates(
                internalAsset, address(vars.collateralAtoken), 0, vars.maxCollateralToLiquidate
            );
        ...
        internal
    {
        ...
        uint256 amountCollateral =
            vars.maxCollateralToLiquidate.rayDiv(collateralReserve.getIndexFromPricePerShare());
        ...
        amountCollateral =
            IGeneralVault(vault).withdrawOnLiquidation(collateralAsset, amountCollateral);
        require(
            amountCollateral >= vars.maxCollateralToLiquidate.percentMul(9900),
     ...
     * - This function needs to be called after all the checks to validate the liquidation have been
     * performed,
     ...
     * liquidation
     * @param debtToCover The debt amount of borrowed `asset` the liquidator wants to cover
     * @param userCollateralBalance The collateral balance for the specific `collateralAsset` of the
     * user being liquidated
     * @return collateralAmount: The maximum amount that is possible to liquidate given all the
