...
contract YearnWETHVault is GeneralVault {
        ...
        returns (uint256)
    {
        ...
        // Withdraw from Yearn Vault and receive WETH
        uint256 assetAmount =
            IYearnVault(_addressesProvider.getAddress("YVWETH")).withdraw(_amount, address(this), 1);
    ...
    function _convertAndDepositYield(address _tokenOut, uint256 _wethAmount) internal {
        ...
        uint256 _minFTMAmount = _wethAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWETH"))
        ).div(oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))).percentMul(9900);

        uint256 minAmountFromPrice = _minFTMAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))
        ).mul(10 ** assetDecimal).div(10 ** 18).div(oracle.getAssetPrice(_tokenOut)).percentMul(
            9900
        );
    ...
    function _processTreasury(uint256 _yieldAmount) internal returns (uint256) {
        ...
        IERC20(_addressesProvider.getAddress("YVWETH")).safeTransfer(
