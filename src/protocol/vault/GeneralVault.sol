...
contract GeneralVault is VersionedInitializable {
    ...
    function depositCollateral(address _asset, uint256 _amount) external payable virtual {
        ...
        // Ex: if user deposit 100ETH, this will deposit 100ETH to Lido and receive 100stETH TODO No
        // Lido
        ...
        ILendingPool(_addressesProvider.getLendingPool()).deposit(
            _stAsset, _stAssetAmount, msg.sender, 0
        );
     ...
     *   different wallet
     */
    function withdrawCollateral(address _asset, uint256 _amount, address _to) external virtual {
        ...
        uint256 _amountToWithdraw = ILendingPool(_addressesProvider.getLendingPool()).withdrawFrom(
            _stAsset, _stAssetAmount, msg.sender, address(this)
        );
        ...
        require(withdrawAmount >= _amount.percentMul(9900), Errors.VT_WITHDRAW_AMOUNT_MISMATCH);
    ...
    function setTreasuryInfo(address _treasury, uint256 _fee) external onlyAdmin {
        ...
        require(_fee <= 3000, Errors.VT_FEE_TOO_BIG);
    ...
    function _getYieldAmount(address _stAsset) internal view returns (uint256) {
        (uint256 stAssetBalance, uint256 aTokenBalance) =
            ILendingPool(_addressesProvider.getLendingPool()).getTotalBalanceOfAssetPair(_stAsset);
    ...
    function _getAssetYields(uint256 _WETHAmount) internal view returns (AssetYield[] memory) {
        ...
        (uint256 totalVolume, uint256[] memory volumes, address[] memory assets, uint256 length) =
