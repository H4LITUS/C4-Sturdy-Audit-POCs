...
contract YieldManager is VersionedInitializable, Ownable {
    ...
    uint256 public constant UNISWAP_FEE = 10_000; // 1%
     ...
     */
    function setCurvePool(address _tokenIn, address _tokenOut, address _pool) external onlyAdmin {
     ...
     *
     */
    function _getAssetYields(uint256 _totalYieldAmount)
        internal
        view
    ...
    function _convertAssetToExchangeToken(address asset, uint256 amount) internal {
        UniswapAdapter.swapExactTokensForTokens(
            _addressesProvider, asset, _exchangeToken, amount, UNISWAP_FEE, SLIPPAGE
        );
    }
        ...
        returns (uint256 receivedAmount)
    {
        ...
        receivedAmount = CurveswapAdapter.swapExactTokensForTokens(
            _addressesProvider, _pool, _exchangeToken, _tokenOut, _amount, SLIPPAGE
        );
    }
