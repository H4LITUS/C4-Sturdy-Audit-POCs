...
contract LidoVault is GeneralVault {
    ...
    /**
     * @dev Withdraw from yield pool based on strategy with stAsset and deliver
     * asset
     */
        ...
        returns (uint256)
    {
        address LIDO = _addressesProvider.getAddress("LIDO");
        if (_asset == address(0)) {
            // Case of ETH withdraw request from user, so exchange stETH -> ETH
            // via curve
            uint256 receivedETHAmount = CurveswapAdapter.swapExactTokensForTokens(
                _addressesProvider,
                _addressesProvider.getAddress("STETH_ETH_POOL"),
            ...
            require(sent, Errors.VT_COLLATERAL_WITHDRAW_INVALID); // @audit-issue
        } else {
