...
contract YearnWBTCVault is GeneralVault {
        ...
        returns (uint256)
    {
        ...
        // Withdraw from Yearn Vault and receive WBTC
        uint256 assetAmount =
            IYearnVault(_addressesProvider.getAddress("YVWBTC")).withdraw(_amount, address(this), 1);
    ...
    function _convertAndDepositYield(address _tokenOut, uint256 _wbtcAmount) internal {
        ...
        uint256 minAmountFromPrice = _wbtcAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWBTC"))
        ).div(10 ** 8).mul(10 ** assetDecimal).div(oracle.getAssetPrice(_tokenOut)).percentMul(9800);
        ...
        uint256[] memory receivedAmounts = IUniswapV2Router02(uniswapRouter)
            ...
            _wbtcAmount, minAmountFromPrice, path, address(this), block.timestamp
        );
    ...
    function _processTreasury(uint256 _yieldAmount) internal returns (uint256) {
        ...
        IERC20(_addressesProvider.getAddress("YVWBTC")).safeTransfer(
