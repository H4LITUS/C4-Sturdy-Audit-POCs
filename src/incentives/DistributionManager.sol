...
contract DistributionManager is ISturdyDistributionManager {
        ...
        internal
    {
        for (uint256 i = 0; i < assetsConfigInput.length; i++) {
            ...
            emit AssetConfigUpdated(
                assetsConfigInput[i].underlyingAsset, assetsConfigInput[i].emissionPerSecond
            );
        }
    }
    ...
    /**
     * @dev Used by "frontend" stake contracts to update the data of an user when claiming rewards
     * from there
     ...
     * @param emissionPerSecond Representing the total rewards distributed per second per asset
     * unit, on the distribution
        ...
        returns (uint256)
    {
        ...
        return emissionPerSecond.mul(timeDelta).mul(10 ** uint256(_PRECISION)).div(totalBalance).add(
            currentIndex
        );
    }
}
