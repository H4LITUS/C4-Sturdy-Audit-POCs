 ...
 * - To be covered by a proxy contract, owned by the LendingPoolAddressesProvider of the specific
 * market
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
        ...
        whenNotPaused
    {
        ...
        if (isCollateral && reserve.yieldAddress != address(0)) {
            isFirstDeposit =
                IAToken(aToken).mint(onBehalfOf, amount, reserve.getIndexFromPricePerShare());
        } else {
        ...
        returns (uint256)
    {
        ...
        if (isCollateral && reserve.yieldAddress != address(0)) {
            IAToken(reserve.aTokenAddress).burn(
                from, to, amountToWithdraw, reserve.getIndexFromPricePerShare()
            );
        ...
        returns (uint256)
    {
        ...
        ValidationLogic.validateRepay(
            reserve, amount, interestRateMode, onBehalfOf, stableDebt, variableDebt
        );
    ...
    /**
     * @dev Returns the percentage of available liquidity that can be borrowed at once at stable
     * rate
     */
    ...
    function _executeBorrow(ExecuteBorrowParams memory vars) internal {
        ...
        uint256 amountInETH = IPriceOracleGetter(oracle).getAssetPrice(vars.asset).mul(vars.amount)
    ...
    function _isInterestRateAvailable(address interestRateStrategyAddress)
        ...
        returns (bool)
    {
        return IReserveInterestRateStrategy(interestRateStrategyAddress).variableRateSlope1() == 0
