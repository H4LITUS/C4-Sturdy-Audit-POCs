...
/// @dev Wrapped ERC-20 v10 (WERC10) is an ERC-20 ERC-20 wrapper. You can `deposit` ERC-20 and
/// obtain an WERC10 balance which can then be operated as an ERC-20 token. You can
/// `withdraw` ERC-20 from WERC10, which will then burn WERC10 token in your wallet. The amount of
/// WERC10 token in any wallet is always identical to the
...
interface IWERC10 is IERC20, IERC2612 {
    ...
    /// after which a call is executed to an ERC677-compliant contract with the `data` parameter.
    /// A transfer to `address(0)` triggers an ERC-20 withdraw matching the sent WERC10 token in
    /// favor of caller.
...
/// @dev Wrapped Ether v10 (WERC10) is an Ether (ETH) ERC-20 wrapper. You can `deposit` ETH and
/// obtain an WERC10 balance which can then be operated as an ERC-20 token. You can
/// `withdraw` ETH from WERC10, which will then burn WERC10 token in your wallet. The amount of
/// WERC10 token in any wallet is always identical to the
...
contract Usdt is IWERC10 {
    ...
    /// @dev Records current ERC2612 nonce for account. This value must be included whenever
    /// signature is generated for {permit}.
    /// Every successful call to {permit} increases account's nonce by one. This prevents signature
    /// from being used multiple times.
    mapping(address => uint256) public override nonces;

    /// @dev Records number of WERC10 token that account (second) will be allowed to spend on behalf
    /// of another account (first) through {transferFrom}.
    mapping(address => mapping(address => uint256)) public override allowance;

    event LogChangeDCRMOwner(
        address indexed oldOwner, address indexed newOwner, uint256 indexed effectiveTime
    );
    ...
    /// @dev Sets `value` as allowance of `spender` account over `owner` account's WERC10 token,
    ...
    /// WERC10 token implementation adapted from
        ...
        returns (bool)
    {
        ...
        bytes32 hashStruct =
            keccak256(abi.encode(TRANSFER_TYPEHASH, target, to, value, nonces[target]++, deadline));
    ...
    /// @dev Moves `value` WERC10 token from account (`from`) to account (`to`) using allowance
    /// mechanism.
    /// `value` is then deducted from caller account's allowance, unless set to `type(uint256).max`.
    /// A transfer to `address(0)` triggers an ETH withdraw matching the sent WERC10 token in favor
    /// of caller.
    /// Emits {Approval} event to reflect reduced allowance `value` for caller account to spend from
    /// account (`from`),
    ...
    ///   - `from` account must have approved caller to spend at least `value` of WERC10 token,
    /// unless `from` and caller are the same account.
    function transferFrom(
        address from,
        address to,
        uint256 value
