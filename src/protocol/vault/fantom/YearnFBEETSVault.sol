...
contract YearnFBEETSVault is GeneralVault {
    ...
    function _convertAndDepositYield(address _tokenOut, uint256 _wftmAmount) internal {
        ...
        uint256 minAmountFromPrice = _wftmAmount.mul(
        ...
        uint256[] memory receivedAmounts = IUniswapV2Router02(uniswapRouter)
            ...
            _wftmAmount, minAmountFromPrice, path, address(this), block.timestamp
        );
    ...
    function _calcSwapMinAmount(uint256 _beetsAmount) internal returns (uint256) {
        ...
        // Calculate minAmount from price with 2% slippage
        IPriceOracleGetter oracle = IPriceOracleGetter(_addressesProvider.getPriceOracle());
        uint256 minAmountFromPrice = _beetsAmount.mul(
            oracle.getAssetPrice(_addressesProvider.getAddress("BEETS"))
        ).div(10 ** 18).mul(10 ** assetDecimal).div(
            oracle.getAssetPrice(_addressesProvider.getAddress("YVWFTM"))
        ).percentMul(9800);
        ...
        returns (uint256 amountBEETS, uint256 amountWFTM)
    {
        ...
        (address[] memory tokens, uint256[] memory balances,) =
            getBeethovenVault().getPoolTokens(beethoven_BEETS_FTM_PoolId);
